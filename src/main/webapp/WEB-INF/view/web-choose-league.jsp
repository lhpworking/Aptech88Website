<%@include file="../common/website/web-header.jspf" %>
 <main class="main-content position-relative border-radius-lg  ">
        <div class="container">
            <div class="league-choose-block p-5">
                <h1 class="text-center pb-5">Choose Your League</h1>
                <div class="search-league text-center pb-5">
                    <div class="row">
                        <div class="col-12">
                            <div class="input-group mb-3">
                                <input type="text" class="form-control form-control-lg" placeholder="Find league.."
                                    aria-label="Find league" aria-describedby="button-addon2">
                                <button class="btn btn-secondary" type="button" id="button-addon2"><i
                                        class="bi bi-search"></i></button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="league-choose-form ">
                    <div class="row mb-3 flex-lg-row flex-column ">
                        <div class="col-lg-4 col-12">
                            <div class="league-choose-group mb-3  d-flex shadow rounded-md overflow-hidden">
                                <button
                                    class="btn btn-outline-dark  d-inline-flex align-items-center border-0  p-3 w-100"
                                    data-bs-toggle="button"> <img class="rounded-circle me-3"
                                        src="https://source.unsplash.com/random" alt="" width="100" height="100">UEFA
                                    CHAMPIONS
                                    LEAGUE</button>
                            </div>
                        </div>
                        <div class="col-lg-4 col-12">
                            <div class="league-choose-group mb-3  d-flex shadow rounded-md overflow-hidden">
                                <button
                                    class="btn btn-outline-dark d-inline-flex align-items-center border-0  p-3 w-100"
                                    data-bs-toggle="button"> <img class="rounded-circle me-3"
                                        src="https://source.unsplash.com/random" alt="" width="100" height="100">UEFA
                                    CHAMPIONS
                                    LEAGUE</button>
                            </div>
                        </div>
                        <div class="col-lg-4 col-12">
                            <div class="league-choose-group mb-3  d-flex shadow rounded-md overflow-hidden">
                                <button
                                    class="btn btn-outline-dark d-inline-flex align-items-center border-0  p-3 w-100"
                                    data-bs-toggle="button"> <img class="rounded-circle me-3"
                                        src="https://source.unsplash.com/random" alt="" width="100" height="100">UEFA
                                    CHAMPIONS
                                    LEAGUE</button>
                            </div>
                        </div>
                    </div>
                    <div class="row mb-3 flex-lg-row flex-column ">
                        <div class="col-lg-4 col-12">
                            <div class="league-choose-group mb-3  d-flex shadow rounded-md overflow-hidden">
                                <button
                                    class="btn btn-outline-dark d-inline-flex align-items-center border-0  p-3 w-100"
                                    data-bs-toggle="button"> <img class="rounded-circle me-3"
                                        src="https://source.unsplash.com/random" alt="" width="100" height="100">UEFA
                                    CHAMPIONS
                                    LEAGUE</button>
                            </div>
                        </div>
                        <div class="col-lg-4 col-12">
                            <div class="league-choose-group mb-3  d-flex shadow rounded-md overflow-hidden">
                                <button
                                    class="btn btn-outline-dark d-inline-flex align-items-center border-0  p-3 w-100"
                                    data-bs-toggle="button"> <img class="rounded-circle me-3"
                                        src="https://source.unsplash.com/random" alt="" width="100" height="100">UEFA
                                    CHAMPIONS
                                    LEAGUE</button>
                            </div>
                        </div>
                        <div class="col-lg-4 col-12">
                            <div class="league-choose-group mb-3  d-flex shadow rounded-md overflow-hidden">
                                <button
                                    class="btn btn-outline-dark d-inline-flex align-items-center border-0  p-3 w-100"
                                    data-bs-toggle="button"> <img class="rounded-circle me-3"
                                        src="https://source.unsplash.com/random" alt="" width="100" height="100">UEFA
                                    CHAMPIONS
                                    LEAGUE</button>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </main>
<%@include file="../common/website/web-footer.jspf" %>
